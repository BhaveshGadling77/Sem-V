...
-- Q3
SELECT i.dept_name, COUNT(i.ID) as instructors FROM instructor as i GROUP BY i.dept_name
ORDER BY instructors DESC;

--Q4
SELECT s.ID, s.name, SUM(c.credits) AS total_credits FROM student AS s JOIN takes AS t ON s.ID
= t.ID JOIN course AS c ON t.course_id = c.course_id WHERE t.grade IS NOT NULL AND t.grade
